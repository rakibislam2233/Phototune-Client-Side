...
const AuthorDetails = () => {
  ...
  useEffect(() => {
    axios.get(`https://phototune-server-side-l7ll.vercel.app/users/${id}`).then((res) => {
      setAuthorDetails(res.data);
    });
  }, [id]);
  useEffect(() => {
    axios
      .get(`https://phototune-server-side-l7ll.vercel.app/hostPhotography/${authorDetails?.email}`)
      .then((res) => {
        setLoading(false);
        setData(res.data);
      });
